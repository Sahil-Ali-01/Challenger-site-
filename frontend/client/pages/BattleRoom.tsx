...
export default function BattleRoom() {
  ...
  const publishProfileStatsUpdate = (payload: { wins: number; losses: number; elo_rating?: number; total_points?: number; weekly_points?: number; streak?: number; total_quizzes?: number; accuracy_percentage?: number; correct_answers?: number; }) => {
    try {
  ...
  useEffect(() => {
    ...
    socket = storedSocket || io(import.meta.env.VITE_SOCKET_URL || import.meta.env.VITE_API_URL || window.location.origin, {
      reconnection: true,
    ...
    try {
      ...
      if (token) {
        ...
        if (response.ok) {
          ...
          const wins = data?.leaderboard?.total_wins ?? 0;
          const losses = data?.leaderboard?.total_losses ?? 0;
          setPlayerStats({ wins, losses });
          publishProfileStatsUpdate({
            wins,
            losses,
            elo_rating: data?.leaderboard?.elo_rating,
            total_points: data?.leaderboard?.total_points,
            weekly_points: data?.leaderboard?.weekly_points,
            streak: data?.leaderboard?.streak,
            total_quizzes: data?.leaderboard?.total_quizzes,
            accuracy_percentage: data?.leaderboard?.accuracy_percentage,
            correct_answers: data?.leaderboard?.correct_answers,
          });
          return;
        }
      }
      ...
      if (!parsedUser?.id || !supabase) {
        console.error('No user or supabase unavailable for fallback stats fetch');
        return;
      }
      ...
      if (profile) {
        ...
        setPlayerStats({ wins, losses });
        publishProfileStatsUpdate({ wins, losses, total_quizzes: wins + losses });
      }
    } catch (error) {
                   ...
                   {currentQuestion.options.map((option, index) => {
                     ...
                     return (
                       <button
                         ...
                         className={cn(
                           "w-full p-5 rounded-2xl text-left font-bold transition-all duration-300 border flex items-start justify-between group min-h-24",
